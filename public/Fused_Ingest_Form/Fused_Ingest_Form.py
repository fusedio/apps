...
input = st.text_input(
    "Input (URL or list of URL)",
    help="A path to file or files on S3 or HTTPS to ingest. Files may be Parquet or another geo data format.",
)
try:
    ...
    path = "s3://NEED_LOGIN/TABLE_NAME"
output = st.text_input(
    "Output (S3 URL)",
    value=path,
    help="Location on S3 to write the table to.",
    placeholder="s3://fused-users/...",
)
if len(output) > 0 and not output.startswith("s3://"):
    st.error("Invalid output. Must start with `s3://`")
partitioning_method = st.selectbox(
    "Partitioning Method",
    ["rows", "area", "length", "coords"],
    help="The method to use for grouping rows into partitions.",
)
target = st.empty()
if st.checkbox("Customize Partitioning", False):
    target_num_chunks = 0  # it will get ignored
    partitioning_maximum_per_file = st.number_input(
        "Partitioning Maximum per File",
        value=100_000,
        step=1,
        min_value=0,
        help="Maximum value for `partitioning_method` to use per file. If `None`, defaults to _1/10th_ of the total value of `partitioning_method`. So if the value is `None` and `partitioning_method` is `area`, then each file will be have no more than 1/10th the total area of all geometries. Defaults to `None`.",
    )
    partitioning_maximum_per_chunk = st.number_input(
        "Partitioning Maximum per Chunk",
        value=5000,
        step=1,
        min_value=0,
        help="Maximum value for `partitioning_method` to use per chunk. If `None`, defaults to _1/100th_ of the total value of `partitioning_method`. So if the value is `None` and `partitioning_method` is `area`, then each file will be have no more than 1/100th the total area of all geometries. Defaults to `None`.",
    )
    ...
    partitioning_force_utm = st.selectbox(
        "Partitioning Force UTM",
        ["chunk", "file", None],
        help="Whether to force partitioning within UTM zones. If set to `file`, this will ensure that the centroid of all geometries per _file_ are contained in the same UTM zone. If set to `chunk`, this will ensure that the centroid of all geometries per _chunk_ are contained in the same UTM zone. If set to `None`, then no UTM-based partitioning will be done. Defaults to `chunk`.",
    )
    if st.checkbox(
        "Subdivide by area",
        False,
        help="The method to use for subdividing large geometries into multiple rows. Currently the only option is `area`, where geometries will be subdivided based on their area (in WGS84 degrees).",
    ):
        subdivide_method = "area"
        subdivide_start = st.number_input(
            "Subdivide Start",
            value=0.0,
            help="The value above which geometries will be subdivided into smaller parts, according to `subdivide_method`.",
        )
        subdivide_stop = st.number_input(
            "Subdivide Stop",
            value=0.0,
            help="The value below which geometries will not be subdivided into smaller parts, according to `subdivide_method`. Recommended to be equal to subdivide_start. If `None`, geometries will be subdivided up to a recursion depth of 100 or until the subdivided geometry is rectangular.",
        )
    else:
        subdivide_method = subdivide_start = subdivide_stop = None
    if st.checkbox(
        "Parition based on lat & lon",
        help="Does the file contain `lat` & `lon` columns instead of a `geometry` column?",
    ):
        lon = st.text_input(
            "Lon column name", "lon", help="Names of longitude, latitude columns to construct point geometries from."
        )
    ...
    target_num_chunks = target.number_input(
        "Target Number of Chunks",
        value=1000,
        help="The target for the number of files if `partitioning_maximum_per_file` is None. ",
    )
...
with st.expander("Additional configuration"):
    ...
    if st.checkbox("Input file Suffix"):
        file_suffix = (
            st.text_input(
                "Input file Suffix (optional)",
                help="Filter which files are used for ingest. If `file_suffix` is not None, it will be used to filter paths by checking the trailing characters of each filename. E.g. pass `file_suffix='.geojson'` to include only GeoJSON files inside the directory.",
            )
            or None
        )
    else:
